        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            for turn in range(5):
                ...
                if response.status_code != 200:
                    print(f"Claude API error turn {turn}: {response.status_code} {response.text}")
                    return None
                ...
                if stop_reason == "tool_use":
                    ...
                    for block in result.get("content", []):
                        if block.get("type") == "server_tool_use":
                            tool_results.append({"type": "server_tool_result", "tool_use_id": block["id"]})
                    if tool_results:
...
async def get_ai_news(ticker: str, company_name: str) -> list | None:
    ...
    prompt = f"""Search the web for the latest news about {company_name} ({ticker}) stock. Find recent earnings results, analyst upgrades/downgrades, product announcements, regulatory developments, and any major events.

Then provide the 6 most important recent news items as a JSON array. Each item must have: headline, summary (2-3 sentences with investment implications), sentiment (positive/negative/neutral), and category (earnings/product/regulatory/market/management/macro/competitive/partnership).
...
Return ONLY a valid JSON array, no other text."""
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            # May need multiple turns if Claude does tool use
            for turn in range(5):
                response = await client.post(
                    "https://api.anthropic.com/v1/messages",
                    headers={"Content-Type":"application/json","x-api-key":CLAUDE_API_KEY,"anthropic-version":"2023-06-01"},
                ...
                if response.status_code != 200:
                    print(f"Claude News API error turn {turn}: {response.status_code} {response.text}")
                    return None
                ...
                # If tool_use, we need to continue the conversation
                if stop_reason == "tool_use":
                ...
                # Other stop reasons - try to extract text anyway
                text = ""
                for block in result.get("content", []):
                    if block.get("type") == "text":
                        text += block.get("text", "")
...
def run_dcf(data: dict, wacc: float, terminal_growth: float, revenue_growth: float, years: int) -> dict:
    ...
    elif upside < -15: recommendation = "SELL"
...
async def lookup_endpoint(req: TickerLookup):
    try:
        ...
        if ai_result:
            assumptions = {"wacc":ai_result["wacc"],"revenue_growth":ai_result["revenue_growth"],"terminal_growth":ai_result["terminal_growth"],"projection_years":ai_result["projection_years"]}
            ai_reasoning = {"wacc":ai_result["wacc_reasoning"],"revenue_growth":ai_result["revenue_growth_reasoning"],"terminal_growth":ai_result["terminal_growth_reasoning"],"projection_years":ai_result["projection_years_reasoning"],"overall":ai_result["overall_analysis"],"powered_by_ai":True}
        else:
            assumptions = {"wacc":round(data["calc_wacc"]*100,2),"revenue_growth":round(data["calc_revenue_growth"]*100,2),"terminal_growth":round(data["calc_terminal_growth"]*100,2),"projection_years":10}
            ai_reasoning = {"wacc":"Calculated using CAPM weighted by capital structure.","revenue_growth":"Based on average historical revenue growth.","terminal_growth":"Standard 2.5% long-term GDP assumption.","projection_years":"Standard 10-year projection.","overall":"AI unavailable. Using formula-based assumptions.","powered_by_ai":False}

        return {
            "company": {
                "name":data["name"],"ticker":data["ticker"],"sector":data["sector"],"industry":data["industry"],
                "price":data["price"],"shares_outstanding":data["shares_outstanding"],
                "market_cap":data["market_cap"],"cash":data["cash"],"debt":data["debt"],
                "pe_ratio":data["pe_ratio"],"ev_ebitda":data["ev_ebitda"],"beta":data["beta"],
                "dividend_yield":data["dividend_yield"],"profit_margin":data["profit_margin"],
                "roe":data["roe"],"roa":data["roa"],
                "fifty_two_high":data["fifty_two_high"],"fifty_two_low":data["fifty_two_low"],
                "hist_revenue":data["hist_revenue"],"hist_ebit":data["hist_ebit"],
                "hist_net_income":data["hist_net_income"],"hist_fcf":data["hist_fcf"],
                "hist_years":data["hist_years"],"hist_margins":data["hist_margins"],
                "price_history":data["price_history"],
            },
            "auto_assumptions":assumptions,
            "ai_reasoning":ai_reasoning,
            "wacc_breakdown":{
                "risk_free_rate":round(data["risk_free_rate"]*100,2),"equity_risk_premium":round(data["equity_risk_premium"]*100,2),
                "beta":data["beta"],"cost_of_equity":round(data["cost_of_equity"]*100,2),
                "cost_of_debt":round(data["cost_of_debt"]*100,2),"weight_equity":round(data["weight_equity"]*100,2),
                "weight_debt":round(data["weight_debt"]*100,2),"tax_rate":round(data["tax_rate"]*100,2),
            },
            "model_inputs":{
                "ebit_margin":round(data["ebit_margin"],2),"da_pct":round(data["da_pct"]*100,2),
...
def dcf_endpoint(req: DCFRequest):
    try:
        ticker = req.ticker.strip().upper()
        if not ticker: raise HTTPException(status_code=400, detail="Ticker is required.")
        data = fetch_financials(ticker)
        result = run_dcf(data, req.wacc, req.terminal_growth, req.revenue_growth, req.projection_years)
        return {
            "company":{"name":data["name"],"ticker":data["ticker"],"sector":data["sector"],"price":data["price"],"shares_outstanding":data["shares_outstanding"],"market_cap":data["market_cap"],"cash":data["cash"],"debt":data["debt"],"pe_ratio":data["pe_ratio"],"ev_ebitda":data["ev_ebitda"],"beta":data["beta"],"hist_revenue":data["hist_revenue"]},
...
async def news_endpoint(req: NewsRequest):
    try:
        ...
        if news:
            ...
            return {"ticker": ticker, "company": name, "news": []}
    except HTTPException: raise
    except Exception as e:
